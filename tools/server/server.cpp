...
// server-cors-proxy.h builds a server_http_proxy declared by server-models.h.
...
int llama_server(common_params & params, int argc, char ** argv, const server_route_extensions & extensions) {
    ...
    const bool is_router_server =
        params.model.path.empty() && params.model.hf_repo.empty() && params.model.docker_repo.empty();
    ...
    responses_routes.create       = routes.post_responses_oai;
    responses_routes.input_tokens = routes.post_responses_tok_oai;
    if (!is_router_server && extensions.responses) {
        try {
            responses_routes = extensions.responses(routes.get_generation_service());
            if (!responses_routes.create || !responses_routes.input_tokens) {
                throw std::invalid_argument("Responses route factory omitted a required handler");
            }
    ...
    if (responses_routes.retrieve) {
        ctx_http.get("/v1/responses/:response_id", ex_wrapper(responses_routes.retrieve));
    }
    if (responses_routes.delete_response) {
        ctx_http.del("/v1/responses/:response_id", ex_wrapper(responses_routes.delete_response));
    }
    if (responses_routes.cancel) {
        ctx_http.post("/v1/responses/:response_id/cancel", ex_wrapper(responses_routes.cancel));
    }
    if (responses_routes.compact) {
        ctx_http.post("/v1/responses/compact", ex_wrapper(responses_routes.compact));
    }
    if (responses_routes.input_items) {
        ctx_http.get("/v1/responses/:response_id/input_items", ex_wrapper(responses_routes.input_items));
    }
    ...
    if (is_router_server) {
        ...
        clean_up = [&ctx_http, &ctx_server, &mcp_mgr, &responses_routes]() {
            SRV_INF("%s: cleaning up before exit...\n", __func__);
            // stop the session GC first, it finalizes live sessions and wakes pending readers
            server_stream_session_manager_stop();
            ctx_http.stop();
            if (responses_routes.shutdown) {
                responses_routes.shutdown();
            }
        ...
        shutdown_handler = [&](int) {
            ...
            ctx_server.terminate();
            // clean_up() cancels and joins Responses workers after start_loop()
            // returns. Joining one here can deadlock if its task still needs the
            // main server loop which this handler interrupted.
        };
    }
