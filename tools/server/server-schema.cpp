...
std::vector<std::unique_ptr<field>> make_llama_cmpl_schema(const common_params & params_base, task_params & params) {
    ...
    add((new field_json("__responses_tool_metadata"))
        ->set_handler([&](field_eval_context & ctx, const json & data) {
            if (!data.at("__responses_tool_metadata").is_object()) {
                return;
            }
    ...
    add((new field_json("__responses_web_search_wrapper"))
        ->set_handler([&](field_eval_context & ctx, const json & data) {
            ctx.params.responses_web_search_wrapper = data.at("__responses_web_search_wrapper").get<std::string>();
        }));

    add((new field_json("__responses_file_search_wrapper"))
        ->set_handler([&](field_eval_context & ctx, const json & data) {
            ctx.params.responses_file_search_wrapper = data.at("__responses_file_search_wrapper").get<std::string>();
        }));
